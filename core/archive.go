...
// SetArchive alters the archive table to match the state of the data at
// the given timestamp, as indicated by a RFC3339 string.
func SetArchive(db *sqlx.DB, schema string, timestamp string) (map[string][]string, error) {
	records, err := ReadRecords(db, schema)
	if err != nil {
		return nil, err
	}
